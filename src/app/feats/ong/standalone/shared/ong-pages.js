import { initialRows, columns, documentStatus, money, parseMoney, escapeHtml as esc, csvCell, months, revenue, } from './ong-data.js';
const icon = (path) => `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${path}"/></svg>`;
const fileIcon = icon('M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z M14 2v6h6');
const tone = (status) => ['Aberta', 'Aberto', 'Ativo', 'Confirmada', 'Recebida', 'Aprovado', 'Válido'].includes(status)
    ? 'green'
    : ['Pendente', 'Vencendo'].includes(status)
        ...
        : ['Cancelado', 'Rejeitado', 'Vencido'].includes(status)
            ...
            : status === 'Em análise'
                ? 'blue'
                : '';
const badge = (status) => `<span class="badge ${tone(status)}">${esc(status)}</span>`;
...
const inputField = (name, label, value = '', type = 'text', extra = '') => `<label class="field">${label}<input name="${name}" type="${type}" value="${esc(value)}" ${extra}><span class="validation-error" hidden></span></label>`;
const selectField = (name, label, choices, selected = '') => `<label class="field">${label}<select name="${name}">${choices.map((value) => `<option ${value === selected ? 'selected' : ''}>${esc(value)}</option>`).join('')}</select></label>`;
const projects = [
...
export function initializeOngPage(root, kind, context) {
    const abort = new AbortController();
    const options = { signal: abort.signal };
    const query = (selector) => root.querySelector(selector);
    const body = query('.page-body');
    const dialog = query('.management-dialog');
    const modalForm = query('.modal-form');
    const feedback = query('.page-feedback');
    ...
    const write = (key, value) => {
        try {
            ...
            announce('Não foi possível salvar no navegador. Verifique o espaço disponível e tente novamente.', true);
            return false;
        }
    };
    try {
    ...
    const update = (row, changes) => persistRows(rows.map((item) => (item.id === row.id ? { ...item, ...changes } : item)));
    const modalError = (message) => {
        const el = query('.modal-error');
        el.textContent = message;
        el.hidden = !message;
    };
    const openModal = (title, html, label, save, danger = false) => {
        disposeUpload?.();
    ...
    const validate = (form) => {
        ...
        form
            ...
            const error = input.closest('.field')?.querySelector('.validation-error');
            if (error) {
    ...
    modalForm.addEventListener('submit', async (event) => {
        ...
        if (!modalSave || !validate(modalForm))
            return;
        const confirm = query('[data-confirm]');
        confirm.disabled = true;
        try {
            if (await modalSave(modalForm))
                dialog.close();
        }
        catch {
    ...
    function setupUpload(container) {
        ...
        const remove = zone.querySelector('[data-remove-file]');
        const error = zone.querySelector('.validation-error');
    ...
    const action = (label, name, row, css = '') => `<button type="button" class="${css}" data-row-action="${name}" data-id="${esc(row.id)}">${label}</button>`;
    function rowActions(row) {
        ...
        if (kind === 'documentos') {
            ...
            return (action('Baixar', 'download', row, 'blue') +
                (row['type'] === 'Relatório'
                    ? ''
                    : action(state === 'Válido' ? 'Atualizar' : 'Renovar', 'renew', row, state === 'Válido' ? '' : tone(state))));
        }
        return '';
    }
    function renderRows() {
        ...
        if (!tableBody)
            return;
        const visible = rows.filter((row) => selected.size === 0 ||
            selected.has(String(kind === 'voluntarios' ? row['interest'] : row.status)));
        if (sortKey)
            visible.sort((a, b) => typeof a[sortKey] === 'number' && typeof b[sortKey] === 'number'
        ...
        tableBody.innerHTML = visible
            .map((row) => `<tr data-id="${esc(row.id)}">${columns[kind].map(([key, label]) => {
            ...
            if (key === 'status')
                value = badge(row.status);
            if (key === 'actions')
                value = `<div class="table-actions">${rowActions(row)}</div>`;
            if (key === 'date') {
    ...
    function tableMarkup() {
        return `<section class="table-card" aria-label="Lista de registros">${kind === 'vagas'
            ? `<div class="tabs" role="group" aria-label="Filtrar vagas">${[
                ['', 'Todos'],
                ['Aberta', 'Abertas'],
                ['Encerrada', 'Encerradas'],
            ]
                .map(([value, label]) => `<button data-filter="${value}" aria-pressed="${!value}" class="${!value ? 'selected' : ''}">${label}</button>`)
                .join('')}</div>`
    ...
    const confirmChange = (row, title, text, status, label, danger = false, reason = false) => {
        openModal(title, `<p>${esc(text)}</p>${reason ? '<label class="field">Justificativa (opcional)<textarea name="reason" maxlength="1000"></textarea></label>' : ''}`, label, (form) => {
            if (!update(row, {
                status,
            ...
            announce(status === 'Aberto' && kind === 'eventos'
    ...
    function openEditor(row) {
        ...
        const heading = row
            ...
            : event
                ? 'Novo Evento'
                : 'Nova Vaga';
        const html = inputField('name', event ? 'Nome do Evento' : 'Título da Vaga', row?.name || '', 'text', 'required maxlength="150"') +
            (event
                ? inputField('date', 'Data', String(row?.['date'] || ''), 'date', 'required') +
                    inputField('capacity', 'Número de vagas', String(row?.['capacity'] || ''), 'number', `required min="${row?.['count'] || 1}" step="1"`)
                : selectField('mode', 'Modalidade', ['Remoto', 'Presencial'], String(row?.['mode'] || 'Remoto'))) +
        ...
        openModal(heading, html, 'Salvar', (form) => {
            ...
            if (/\/(nova|novo|editar\/)/.test(context.path))
                context.navigate('/ong/' + kind);
            return true;
        });
    }
    function openDocument(row) {
        ...
        let upload;
        openModal(title, uploadMarkup +
            selectField('type', 'Tipo de documento', ['Estatuto', 'Ata', 'Certidão', 'Certificado', 'Relatório', 'Outro'], String(row?.['type'] || 'Estatuto')) +
            inputField('date', renewing ? 'Nova data de vencimento' : 'Data de vencimento (opcional)', String(row?.['date'] || ''), 'date', renewing ? 'required' : ''), renewing ? 'Renovar documento' : row ? 'Atualizar' : 'Enviar', (form) => {
            if (upload.pending()) {
    ...
    function renderAccounting() {
        ...
        form.addEventListener('submit', (event) => {
            event.preventDefault();
            const value = parseMoney(amount.value);
            amount.setCustomValidity(!Number.isFinite(value) || value <= 0 ? 'Informe um valor maior que zero.' : '');
            if (!validate(form))
                return;
            if (upload.pending()) {
        ...
        body.innerHTML =
            kpis(true) +
                `<section class="charts" aria-label="Gráficos de desempenho"><figure class="chart-card"><figcaption>Arrecadação Mensal (Últimos 6 Meses)</figcaption><div class="bar-chart" role="img" aria-label="Abril: R$ 3.100; Maio: R$ 5.200; Junho: R$ 4.500; Julho: R$ 7.800; Agosto: R$ 8.900; Setembro: R$ 11.200"><div class="y-axis"><span>12K</span><span>9K</span><span>6K</span><span>3K</span><span>0</span></div><div class="bars">${revenue.map((value, i) => `<div class="bar" tabindex="0" style="--height:${value / 120}%" title="${months[i]}: ${money(value)}" aria-label="${months[i]}: ${money(value)}"><span class="value">${value.toLocaleString('pt-BR')}</span><span class="month">${months[i]}</span></div>`).join('')}</div></div></figure><figure class="chart-card"><figcaption>Distribuição por Categoria</figcaption><svg class="donut" viewBox="0 0 160 160" role="img" aria-label="Alimentação e Cozinha 50%; Livros e Educação 28%; Infraestrutura e Espaço 22%"><g transform="rotate(-90 80 80)">${[
                    ...
                    .map(([color, value, offset, label]) => `<circle tabindex="0" cx="80" cy="80" r="60" fill="none" stroke="${color}" stroke-width="30" pathLength="100" stroke-dasharray="${value} ${100 - Number(value)}" stroke-dashoffset="-${offset}"><title>${esc(label)}: ${value}% · ${money((45200 * Number(value)) / 100)}</title></circle>`)
                    .join('')}</g></svg><ul class="legend">${[
                    ['#0259e1', 'Alimentação & Cozinha', 50],
                    ['#05bf97', 'Livros & Educação', 28],
                    ['#08c4e7', 'Infraestrutura & Espaço', 22],
                ]
                    .map(([color, label, value]) => `<li><i style="--dot:${color}"></i>${esc(label)}<strong>${value}%</strong></li>`)
    ...
    function renderSettings() {
        ...
        const switches = (labels, prefix) => `<div class="switch-list">${labels
            .map((label, i) => {
            const key = prefix + i;
            const checked = typeof preferences[key] === 'boolean'
                ? preferences[key]
    ...
    function settingsAction(actionName) {
        if (actionName === 'password') {
            const passwordField = (name, label) => `<label class="field">${label}<span class="password-wrap"><input name="${name}" type="password" required ${name === 'current' ? 'autocomplete="current-password"' : 'minlength="8" autocomplete="new-password"'}><button type="button" data-eye="${name}" aria-label="Mostrar ${label.toLowerCase()}" aria-pressed="false">Mostrar</button></span><span class="validation-error" hidden></span></label>`;
            openModal('Alterar Senha', passwordField('current', 'Senha atual') +
                passwordField('next', 'Nova senha') +
                passwordField('confirmation', 'Confirmar nova senha'), 'Salvar nova senha', (form) => {
                const data = new FormData(form);
                if (data.get('next') !== data.get('confirmation')) {
                    modalError('A confirmação deve coincidir com a nova senha.');
                    return false;
                }
                const result = context.changePassword(String(data.get('current')), String(data.get('next')));
                if (result) {
                    modalError(result);
                    return false;
                }
            ...
            openModal('Desativar conta da organização', `<p>⚠ Esta ação é irreversível. Todos os dados de voluntários, doações e projetos serão permanentemente apagados. Digite 'DESATIVAR' para confirmar.</p>${inputField('confirmation', 'Confirmação', '', 'text', 'required autocomplete="off"')}`, 'Confirmar desativação', (form) => {
                if (new FormData(form).get('confirmation') !== 'DESATIVAR')
                    return false;
                const keys = Object.keys(localStorage).filter((key) => key.startsWith(`movune:ong:${context.owner}:`) ||
                ...
                if (Array.isArray(activities))
                    localStorage.setItem('movune:atividades', JSON.stringify(activities.filter((record) => record.ownerId !== context.owner)));
            ...
            const confirm = query('[data-confirm]');
            confirm.disabled = true;
            query('.modal-content input').addEventListener('input', (event) => {
                confirm.disabled = event.target.value !== 'DESATIVAR';
            }, options);
        }
    }
    ...
    function exportReport(format) {
        ...
        if (format === 'excel') {
            ...
            download(data.map((row) => row.map(csvCell).join(';')).join('\r\n'), 'relatorio-movune.csv', 'text/csv;charset=utf-8');
    ...
    root.addEventListener('click', (event) => {
        ...
        if (eye) {
            const input = modalForm.elements.namedItem(eye.dataset['eye']);
        ...
        if (settings)
            settingsAction(settings.dataset['settings']);
        const exportButton = target.closest('[data-export]');
        if (exportButton)
            exportReport(exportButton.dataset['export']);
        if (target.closest('[data-action="new"]')) {
            if (kind === 'vagas' || kind === 'eventos')
                ...
                query('.accounting-form [name="name"]').focus({ preventScroll: true });
            }
        }
        ...
        switch (button.dataset['rowAction']) {
            case 'edit':
                ...
                break;
            case 'close':
                confirmChange(row, 'Encerrar vaga', 'Tem certeza que deseja encerrar esta vaga?', 'Encerrada', 'Encerrar');
                break;
            case 'reopen':
                confirmChange(row, 'Reabrir vaga', 'Deseja reabrir esta vaga?', 'Aberta', 'Reabrir');
                break;
            case 'approve':
                confirmChange(row, 'Aprovar inscrição', `Deseja aprovar ${row.name} como voluntário(a)?`, 'Ativo', 'Aprovar');
                break;
            case 'reject':
                confirmChange(row, 'Rejeitar inscrição', `Deseja rejeitar a inscrição de ${row.name}?`, 'Rejeitado', 'Rejeitar', true, true);
                break;
            case 'reactivate':
                confirmChange(row, 'Reativar voluntário', `Deseja reativar ${row.name}?`, 'Ativo', 'Reativar');
                break;
            case 'cancel':
                confirmChange(row, 'Cancelar evento', `Tem certeza que deseja cancelar o evento '${row.name}'? Esta ação não poderá ser desfeita.`, 'Cancelado', 'Cancelar evento', true);
                break;
            case 'publish':
                confirmChange(row, 'Publicar evento', `Deseja publicar o evento '${row.name}'? Ele ficará visível para todos os usuários.`, 'Aberto', 'Publicar');
                break;
            case 'profile':
                context.navigate(`/voluntario/${encodeURIComponent(row.id)}/perfil`);
                break;
            case 'message':
                openModal(`Mensagem para ${row.name}`, '<label class="field">Mensagem<textarea name="message" required maxlength="2000"></textarea><span class="validation-error" hidden></span></label>', 'Enviar', (form) => {
                    ...
                    announce('Mensagem registrada no navegador. O envio externo ainda não está conectado.');
                    return true;
                });
                break;
            case 'detail':
                openModal('Detalhes da doação', `<p><strong>${esc(row.name)}</strong></p><p>${esc(row['type'])} · ${typeof row['value'] === 'number' ? money(row['value']) : esc(row['value'])}</p><p>Destino: ${esc(row['project'])}<br>Data: ${dateLabel(String(row['date']), true)}<br>Status: ${esc(row.status)}</p><p>Método de pagamento, comprovante e mensagem não informados.</p>`, row.status === 'Pendente' ? 'Confirmar recebimento' : 'Fechar', row.status === 'Pendente'
                    ? () => {
    ...
    if (editId && (kind === 'vagas' || kind === 'eventos')) {
        ...
        if (row)
            ...
            announce('Registro não encontrado.', true);
    }
    ...
    if (kind === 'voluntarios' && volunteerId) {
        ...
        if (row)
            openModal(row.name, `<p>Área de interesse: ${esc(row['interest'])}</p><p>Disponibilidade: ${esc(row['availability'])}</p><p>Status: ${esc(row.status)}</p>`, 'Fechar');
        else
            announce('Voluntário não encontrado.', true);
    }
    return () => {
        abort.abort();
        observer?.disconnect();
